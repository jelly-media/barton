...
class ModulaDB {
	...
	public function getGalleryById($id) {
		...
		if(! isset($data->wp_field_title))
			$data->wp_field_title = 'title';
		if(! isset($data->wp_field_caption))
			$data->wp_field_caption = 'caption';
		if(! isset($data->randomFactor))
			$data->randomFactor = .5;
	...
	public function getImagesByGalleryId($gid) {
		global $wpdb;
		$imageResults = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->ModulaImages WHERE gid = %d ORDER BY sortOrder ASC", $gid));
		return $imageResults;
	}
