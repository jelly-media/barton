...
class WonderPlugin_Popup_Model {
	...
	function save_to_local_and_email_notify($id, $data, $savetolocal, $emailnotify, $emailto, $emailsubject) {
	...
	function get_localrecord_data($id, $daterange, $customstart, $customend, $numperpage, $page, $nolimit) {
		...
		if ( empty($items) )
		{
			return array(
					"success" => false,
					"message" => 'No record found. Please make sure you have enabled the option "Save to the WordPress Database of this Website" in the popup editor, Email Service tab.'
			);
		}
	...
	function generate_body_code($id, $preview) {
		...
		if ($item_row != null)
		{
			...
			$bool_options = array('uniquevideoiframeid', 'loggedinonly', 'status', 'fullscreen', 'overlayclose', 'showclose', 'closeshadow', 'showprivacy', 'showribbon', 'showclosetip', 'showgrecaptcha', 'showemail', 'showname', 'showfirstname', 'showlastname', 'showcompany', 'showphone', 'showzip', 'showaction', 'showcancel', 'cancelastext', 'videoautoplay', 'videoautoclose',
			...
			else
			{
				...
				$iframevideo = '<iframe class="wonderplugin-box-videoiframe" id="wonderplugin-box-videoiframe' . ((isset($data->uniquevideoiframeid) && $data->uniquevideoiframeid) ? ('-' . $id) : '') . '" src="' . esc_url($videosrc) . '" frameborder="0" allowfullscreen></iframe>';
				$htmlcode = str_replace('__VIDEO__', $iframevideo, $htmlcode);
				...
				$ret .= '<style type="text/css">' . $csscode . '</style>';
	...
	function add_popup_to_page() {
		...
		foreach ($rows as $row)
		{
			...
			if ( $this->check_display_rules($row['id'], $data) )
			{
