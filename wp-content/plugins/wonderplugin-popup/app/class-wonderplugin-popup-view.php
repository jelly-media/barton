...
class WonderPlugin_Popup_View {
	...
	function add_metaboxes() {
		add_meta_box('overview_features', __('WonderPlugin Popup Plugin Features', 'wonderplugin_popup'), array($this, 'show_features'), 'wonderplugin_popup_overview', 'features', '');
	...
	function print_overview() {
		...
		<h2><?php echo __( 'WonderPlugin Popup', 'wonderplugin_popup' ) . ( (WONDERPLUGIN_POPUP_VERSION_TYPE == "C") ? " Commercial Version" : " Free Version") . " " . WONDERPLUGIN_POPUP_VERSION; ?> </h2>
	...
	?>
        ...
        <ul class="wonderplugin-tab-buttons-horizontal-xml" data-panelsid="wonderplugin-popup-settings-panels" id="wp-settings-toolbar">
        	...
        	<li class="wonderplugin-tab-button-horizontal-xml">Email Notification</li>
        </ul>
	...
	function print_register() {
		...
		function validateLicenseForm() {
			...
			if ($.trim($("#wonderplugin-popup-key").val()).length <= 0)
			{
	...
	function print_localrecord() {
		...
		<h2><?php _e( 'WordPress Database', 'wonderplugin_popup' ); ?></h2>
		
		<p><b>Display subscrpition data saved in the WordPress database of this website</b></p>
		...
		{			
			if ( $ret['success'] )
			{
				...
				{
					echo '<h3>No record found. Please make sure you have enabled the option "Save to the WordPress Database of this Website" in the popup editor, Email Service tab.</h3>';
				}
			}
			else
			{
	...
	function prepare_save_post($post) {
		...
		$bool_options = array('uniquevideoiframeid', 'enableretarget', 'loggedinonly', 'status', 'fullscreen', 'overlayclose', 'showclose', 'closeshowshadow', 'showprivacy', 'showribbon', 'showclosetip', 'showgrecaptcha', 'showemail', 'showname', 'showfirstname', 'showlastname', 'showcompany', 'showphone', 'showzip', 'showaction', 'showcancel', 'cancelastext', 'videoautoplay', 'videoautoclose',
				...
				'emailnotify', 'savetolocal', 'mailchimpdoubleoptin', 'icontactdoubleoptin', 'infusionsoftdoubleoptin', 'getresponseautoresponder',
