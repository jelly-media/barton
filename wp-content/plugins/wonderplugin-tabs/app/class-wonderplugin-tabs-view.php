...
class WonderPlugin_Tabs_View {
	...
	?>
		...
		$supportajaxloading = $settings['supportajaxloading'];
		...
		<tr>
			<th>AJAX loading</th>
			<td><label><input name='supportajaxloading' type='checkbox' id='supportajaxloading' <?php echo ($supportajaxloading == 1) ? 'checked' : ''; ?> /> Allow AJAX tab content loading</label>
			<p><label><input name='ajaxverifynonce' type='checkbox' id='ajaxverifynonce' <?php echo ($ajaxverifynonce == 1) ? 'checked' : ''; ?> /> Verify nonce in Ajax</label></p>
			</td>
		</tr>
	...
	function print_register() {
		...
		function validateLicenseForm() {
			...
			if ($.trim($("#wonderplugin-tabs-key").val()).length <= 0)
			{
		...
		?> <div style="padding-left:10px;padding-top:12px;"> <?php
			if (empty($key)) { ?>
				...
				<tr>
					...
					<td><input name="wonderplugin-tabs-key" type="text" id="wonderplugin-tabs-key" value="" class="regular-text" /> <input type="submit" name="save-tabs-license" id="save-tabs-license" class="button button-primary" value="Register"  />
					</td>
				</tr>
				<tr>
					<th></th>
	...
	function save_item_post($item_post) {
		...
		else
		{
			...
			add_filter('safe_style_css', 'wonderplugin_tabs_css_allow');
			add_filter('wp_kses_allowed_html', 'wonderplugin_tabs_tags_allow', 'post');
			...
			remove_filter('wp_kses_allowed_html', 'wonderplugin_tabs_tags_allow', 'post');
			remove_filter('safe_style_css', 'wonderplugin_tabs_css_allow');
				...
				if (empty($data)) {
					...
					<form method="post"
						...
						<p>
							...
							<label><input type="checkbox" name="exportplaylist" value=1>Export content list to an XML file</label>
						</p>
						<p class="submit">
